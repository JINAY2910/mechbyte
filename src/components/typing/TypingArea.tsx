...
export function TypingArea({ text, input, status }: TypingAreaProps) {
  ...
  if (mode === "zen") {
    return (
      <div
        ref={zenScrollRef}
        className="relative h-[6.4rem] md:h-[7.8rem] w-full overflow-y-auto overflow-x-hidden text-left text-[1.25rem] md:text-[1.65rem] leading-relaxed text-[var(--color-text-muted)] select-none"
        style={{ fontFamily }}
      >
        {input ? (
  ...
  return (
    <div
      ref={viewportRef}
      className="relative h-[6.4rem] md:h-[7.8rem] w-full overflow-hidden text-left text-[1.25rem] md:text-[1.65rem] leading-relaxed select-none"
      style={{ fontFamily }}
